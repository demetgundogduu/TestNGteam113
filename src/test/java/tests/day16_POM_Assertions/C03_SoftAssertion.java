...
public class C03_SoftAssertion {
    ...
    public void hardAssertion(){
     ...
     public void softAssertionTesti(){
        /*
        SoftAssert classı sayesınde
        assertAll()'nun calısacagı satıra kadar
         faıled olan olsa bıle,tum assertıonlar calısmaya devam eder

         assertAll() calıstıgında
         failed olan assertıonlar lara ait raporu verır
         Ancak hangı satırda olduklarını vermez

         Selenıum hata mesajı da assertAll() un bulundugu satırı ısaret eder
         Failed olan assertıonları kolaylıkla bulabılırız
